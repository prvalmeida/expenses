...
**Receipt QR scan:** The "Link da Nota" mode can read the NFC-e QR with the phone camera (`hooks/useQrScanner.ts` + jsQR via dynamic import, overlaid by `components/QrScannerModal.tsx`); a successful scan fills the URL field and parses immediately. The SEFAZ allowlist that gates the scan lives in `lib/utils/sefazUrl.ts` — pure, so the client imports it without pulling `receiptService` (mongoose/pdf-parse) into the browser; `receiptService` re-exports the same function, so both API surfaces keep one guard. Do not re-approximate the list in the client.
...
- **Pluggy's `amount` sign convention is not uniform — it depends on the account kind, which is why `deriveDirection` takes the account.** On a CREDIT account a purchase is `type: 'DEBIT'` with a **positive** amount (`NETFLIX.COM` +44,90) and a refund is `type: 'CREDIT'` negative (`AJUSTE CRED` −0,05); on a BANK account a payment is `DEBIT` **negative** (−516,91) and a deposit is `CREDIT` positive. On a card statement the amount is the size of the charge against the bill; on an account it is the effect on the balance. The cross-check between `tx.type` and the sign therefore branches on `kind` — applying BANK's convention to a card made every purchase disagree and land as `status: 'anomaly'`, so no card expense could be imported at all (71 of 86 rows in production). The account parameter is **required, never optional**, or a future caller silently re-arms this, and `PluggyAccountLike.kind` is narrowed to `'BANK' | 'CREDIT'` so the branch is compiler-checked. These shapes are observed from live Caixa accounts, not inferred.
- **A per-account sync error comes back inside a `200` body, so something must actually read it.** `syncAllAccounts` collects each account's failure into `accounts[].error` rather than throwing, so one broken bank cannot stop the others — which is also how a Pluggy endpoint deprecation (410 on every account) ran unnoticed in production: the cron saw HTTP 200 and the review screen only rendered an error for a non-OK response, so a total outage read as a clean sync. The failure is now logged server-side and surfaced in `PluggySync`. For the same reason a test must assert on the *count*, not the shape: `expect(fetched).to.be.a('number')` passed throughout the outage, because `0` is a number.
- **Transactions are read through `GET /v2/transactions` with cursor pagination, and the page cap is a failure, not an exit.** The page-based `GET /transactions` was retired and answers 410; v2 renames `from`/`to` to `dateFrom`/`dateTo`, rejects `pageSize` outright, fixes the page at 500 and paginates by cursor. Two rules follow. A follow-up request carries **only the URL-decoded `after`** parsed out of `next`, never the whole `next` string (that is a 400). And termination is `next === null` **only** — a short page may still have a successor, so the old `rows.length < PAGE_SIZE` test silently dropped every row after one. `drainCursor` reports `truncated` when it hits `MAX_SYNC_PAGES` with a cursor still open, and `syncAccount` throws on it: advancing `lastSyncedAt` after a partial read puts the unread rows permanently outside every future window, since the next window starts at `lastSyncedAt − OVERLAP_DAYS`.
