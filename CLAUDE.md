...
- **`mapPluggyTransaction` narrows `tx.date` to `YYYY-MM-DD`.** Pluggy sends a full ISO timestamp. `PluggyTransaction.date` is compared lexically against a cursor row's date by `listPluggyTransactions`, compared against the stored value by `upsertTransaction` to detect post-import drift, and reaches `Expense.date` through `buildExpenseDocuments` — storing the timestamp makes the drift check fire on a re-read that only moved the clock, and files the expense under a date nothing else can match. `merchantName` falls back `merchant.name` → `merchant.businessName` and treats an empty string as absent; live rows carry either field blank.
