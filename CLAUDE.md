...
- **Transactions come from `GET /v2/transactions`, which is cursor-paginated.** The page-numbered `GET /transactions` was retired and answers `410 This endpoint is deprecated` — every sync failed as `UPSTREAM_FAILED` and staged nothing. v2 renames the date filters (`dateFrom`/`dateTo`, not `from`/`to`), rejects `pageSize` outright (its page size is fixed at 500), and ends a walk by dropping `next` rather than by returning a short page — so `syncAccount` loops on the cursor, never on `rows.length < PAGE_SIZE`. `next` arrives as a ready-made query string (`?accountId=…&after=…`); `client.ts` extracts its `after` value into `nextCursor` so `buildUrl` stays the only place a Pluggy URL is assembled.
- **The amount sign convention is inverted between BANK and CREDIT accounts, so `deriveDirection` takes the account.** On a BANK account an outflow is negative (`DEBIT -5300`). On a CREDIT account the balance being described is the *bill*, so a purchase is a **positive** `DEBIT` (`DEBIT 34.52`) and a refund or bill payment is a **negative** `CREDIT`. Reading it as one rule made the sign cross-check disagree with `tx.type` on every card row, which is precisely the `anomaly` short-circuit in `deriveStaging` — 102 of 114 live rows staged as anomalies and nothing was ever importable. The cross-check itself stays: it is the guard against booking an income as an expense.
- **`mapPluggyTransaction` narrows `tx.date` to `YYYY-MM-DD`.** Pluggy sends a full ISO timestamp. `PluggyTransaction.date` is compared lexically against a cursor row's date by `listPluggyTransactions`, compared against the stored value by `upsertTransaction` to detect post-import drift, and reaches `Expense.date` through `buildExpenseDocuments` — storing the timestamp makes the drift check fire on a re-read that only moved the clock, and files the expense under a date nothing else can match. Rows staged before the narrowing hold a full timestamp, so `002-pluggy-transaction-dates` rewrites them and the drift check normalizes the stored side too — a collection holding both formats breaks `listPluggyTransactions`' lexical keyset pagination, and flips every already-imported row inside the overlap window to `anomaly`. `merchantName` falls back `merchant.name` → `merchant.businessName` and treats an empty string as absent; live rows carry either field blank.
- **The installment anchor date is not the row's date, unlike the bill import's.** A fatura row's date already *is* the original purchase date, so `billService` passes it straight through. A Pluggy row's date is the **posting** date of the one installment it represents — passing it unchanged would file a mid-series purchase in the wrong month. `anchorPurchaseDate` (`pluggyUtils.ts`) resolves the anchor in two steps before calling the same `buildExpenseDocuments` the bill import uses:
  - **Pluggy's own `creditCardMetadata.purchaseDate` wins whenever it is present** (stored as `PluggyTransaction.purchaseDate`), on installment and single-charge rows alike — it *is* the "date you decided to spend" that `Expense.date` means. It is rejected only when it postdates the posting date, which cannot happen for a real charge and would file the expense in a future month.
  - **The month arithmetic is the fallback** for rows and connectors that omit it: back off by `installmentCurrent - 1` months (`addMonthsClamped` handles the negative offset and clamps a day-31 anchor to the target month's last valid day). It reconstructs a purchase *month*, not a calendar day, and is measurably worse — across 27 live Caixa installment rows it missed the reported date on every one, by 2–4 weeks typically and by 3 months at worst. `installmentCurrent` is therefore still required to expand a row that has no `purchaseDate` — one with a total but no plausible current stays `pending` rather than being expanded from an unknown offset. `resolveInstallmentPlan` (`pluggyUtils.ts`) is the single place that decides it, shared by `autoImportStaged` and `importStaged`: with a usable `purchaseDate` the anchor and the total determine the whole series, so only a row missing *both* signals is parked.
...
**`subtype` is mandatory on the expense payloads only.** `createExpenseSchema`/`updateExpenseSchema` require it alongside `type`, so a caller composing an expense must classify it — the UI's subtype select is `required` to match, and warns when the chosen category has no subtypes to offer (a category created without any cannot satisfy the field). It stays optional or nullable everywhere else, and those are not oversights: the bill/receipt import schemas accept an unclassified row, `buildExpenseDocuments` accepts an absent subtype because `billService` drops one that is invalid for its type and still imports, the list-filter schema uses it as an optional filter, and `BaseExpense.subtype` stays optional because stored documents legitimately lack it (pre-existing records and import rows). Requiring it is a boundary rule, not an invariant of the document.
...
**Mongoose drops `undefined` keys from an update, so assigning one is not a clear.** `updateExpense` `$unset`s the fields an edit may drop (`subtype`, plus `cardBrand`/`installment`/`totalInstallments` on the credit → non-credit switch) instead of `$set`ting them to `undefined` — otherwise a PUT that moves a record to another type keeps the old `subtype` and orphans it. `$unset` must be omitted entirely when empty; Mongo rejects `{$unset: {}}`. The v1 boundary no longer reaches the `subtype` branch now that the field is mandatory there, but the internal PUT routes (`/api/expenses/[id]`, `/api/expenses/installment-group`) are unvalidated and do — do not delete it as dead code.
