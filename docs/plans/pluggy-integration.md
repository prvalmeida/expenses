...
Non-installment duplicates are impossible at the staging level because `pluggyId` is unique; the
`skipped_existing` guard in step 12 applies only to expanded installment rows.
...
bank does not stop the others. Wrap the whole run in a Mongo lock: a single `PluggySyncLock` doc
with `_id: 'singleton'`. Acquire with `findOneAndUpdate({ _id: 'singleton', acquiredAt: { $lt: threshold } },
{ $set: { acquiredAt: now } }, { upsert: true })`; on a duplicate-key error the current run exits
quietly. Two overlapping cron firings must not both page the same account. Returns
...
- **Own transfer**: `paymentData.payer`/`receiver` document in a configured own-CPF list
  ...
  `PluggyAccount` → `ignored`, both legs. Both depend on the spike in step 1 confirming that
  Pluggy exposes reliable CPF/CNPJ documents for the counterparty.
...
> **The reconstructed purchase date is approximate.** `addMonthsClamped` clamps day 31 to the last
> valid day of the target month, so walking forward again with `addMonthsClamped(purchaseDate,
> installmentCurrent - 1)` may not reproduce the original `row.date` exactly. That is acceptable
> for an expense tracker, but the plan's unit tests and the review screen should treat the date as
> "month of purchase", not "exact calendar day of purchase".
...
`connectedAt` bounds the fetch window (step 4), so nothing predating the link is ever imported.

> **This cutover is an operational rule, not code.** The PDF import route has no knowledge of
> `PluggyAccount.connectedAt`, so nothing prevents a user from uploading a fatura PDF that overlaps
> with an already-enabled Pluggy account. The rule must be documented in step 20 and trained:
> once a card's Pluggy account is enabled, stop importing that card's fatura PDFs for periods on or
> after `connectedAt`. Reconcile one closed month by hand before relying on it.
...
the redacted samples from step 1. No DB, so it runs on the every-push CI path. Include at least
two boundary cases for the anchor date: a January posting that backs into the previous December,
and a day-31 posting that clamps the reconstructed purchase month.
...
9. **Advisory lock scheme clarified** (step 6) — uses a single `PluggySyncLock` doc with
   `_id: 'singleton'` rather than the migration-ledger pattern, so stale locks are identified by
